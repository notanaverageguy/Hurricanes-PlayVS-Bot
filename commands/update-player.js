...
module.exports = {
	...
	async execute(interaction) {
        ...
        if(interaction.options.getString("team") != null) player.team = interaction.options.getString("team");
        if(interaction.options.getString("role") != null) player.role = interaction.options.getString("role");
        if(interaction.options.getInteger("games_won") != null) player.games_won = interaction.options.getInteger("games_won");
        if(interaction.options.getInteger("games_lost") != null) player.games_lost = interaction.options.getInteger("games_lost");
        player.games_played = player.games_won + player.games_lost;

        await db.collection('Players').update(playerID, player);
		interaction.reply(`Successfully updated player **${player.first_name}**`)
	},
};
